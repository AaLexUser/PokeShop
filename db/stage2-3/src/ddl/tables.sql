...
CREATE TABLE IF NOT EXISTS orders(
    id          serial           PRIMARY KEY,
    UserID      integer          NOT NULL REFERENCES _user(id) ON DELETE CASCADE,
    status    varchar(255)     check (status in ('PROCESSING','DONE','PAID','CANCELLED')),
    totalPrice  integer          NOT NULL CHECK ( totalPrice >= 0 ) DEFAULT 0,
    orderDate   timestamp     DEFAULT CURRENT_DATE CHECK (orderDate <= CURRENT_DATE)
);

CREATE TABLE IF NOT EXISTS orderitems(
    orderID     integer          NOT NULL REFERENCES orders(id) ON DELETE CASCADE,
    itemID      integer          NOT NULL REFERENCES entities(id) ON DELETE CASCADE,
    price       integer          NOT NULL CHECK ( price >= 0),
    quantity    integer          NOT NULL CHECK ( quantity >= 0),
